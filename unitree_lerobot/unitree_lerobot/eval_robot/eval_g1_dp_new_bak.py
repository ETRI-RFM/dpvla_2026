...
def eval_policy(cfg: Config) -> None:
    ...
    try:
        ...
        # Decide the task instruction sent to the policy.
        # Priority: --task_text > --prompt_path file > dataset task.
        task_text = None
        if cfg.task_text:
            ...
            try:
                ...
                if task_text:
                    logger_mp.info(f"Read task instruction from {cfg.prompt_path}")
            except FileNotFoundError:
                logger_mp.warning(
                    f"prompt_path not found: {cfg.prompt_path}; will try dataset task."
                )
                task_text = None
        if task_text is None and cfg.episode_idx is not None:
            # Fetch task from the same parquet row (dataset-friendly path).
            from unitree_lerobot.eval_robot.utils.init_pose import (
                load_init_state_from_parquet as _stub,  # already imported; silence linters
            )
            # Tasks live in meta/episodes parquet — read it directly to avoid
            # a slow LeRobotDataset load.
            import pyarrow.parquet as pq
            import glob, os
            ep_files = sorted(glob.glob(os.path.join(cfg.init_root, "meta/episodes/chunk-*/file-*.parquet")))
            task_text = None
            for f in ep_files:
                t = pq.read_table(
                    str(f),
                    columns=["episode_index", "tasks"],
                    filters=[("episode_index", "=", cfg.episode_idx)],
                )
                if t.num_rows > 0:
                    raw = t.column("tasks").to_pylist()[0]
                    task_text = raw[0] if isinstance(raw, list) and raw else str(raw)
                    break
            if task_text is None:
                logger_mp.warning(
                    f"Could not find task text for episode {cfg.episode_idx} in {cfg.init_root}."
                )

        if not task_text:
            task_text = ""
            logger_mp.warning(
                "Empty task instruction (no --task_text, no readable --prompt_path, "
                "no dataset task). Sending empty string."
            )
        logger_mp.info(f"Task instruction: {task_text!r}")
