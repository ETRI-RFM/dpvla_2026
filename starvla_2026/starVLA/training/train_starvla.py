...
class _FullResumeStepTracker:
    """Stateful step counter so accelerator.save_state/load_state persists the
    completed-step count alongside optimizer/scheduler/RNG.
    """
    def __init__(self):
...
class VLATrainer(TrainerUtils):
    def __init__(self, cfg, model, vla_train_dataloader, optimizer, lr_scheduler, accelerator):
        ...
        self.completed_steps = 0
        self.save_full_state = bool(getattr(cfg.trainer, "save_full_state", False))
        self._full_state_resume_dir = None
        self._step_tracker = _FullResumeStepTracker()
    ...
    def prepare_training(self):
        ...
        self._setup_full_state_resume()

        self._reseed_dataset_for_resume()
    ...
    def _init_checkpointing(self):
        ...
        if is_resume and self.save_full_state and self._find_latest_state_dir() is not None:
            self._full_state_resume_dir = self._find_latest_state_dir()
            self.completed_steps = int(os.path.basename(self._full_state_resume_dir).split("_")[-1])
            logger.info(
                f"[full-resume] will restore optimizer/scheduler/RNG/step from "
                f"{self._full_state_resume_dir} after accelerator.prepare (step {self.completed_steps})"
            )
        elif is_resume:
    ...
    def _adjust_lr_scheduler_for_resume(self):
        ...
        if self.completed_steps > 0 and not (self.save_full_state and self._full_state_resume_dir is not None):
    ...
    def _find_latest_state_dir(self):
        if not os.path.isdir(self.checkpoint_dir):
            return None
        found = []
        for name in os.listdir(self.checkpoint_dir):
            full = os.path.join(self.checkpoint_dir, name)
            if os.path.isdir(full) and name.startswith("state_"):
                try:
                    found.append((int(name.split("_")[-1]), full))
                except ValueError:
                    pass
        return max(found)[1] if found else None

    def _setup_full_state_resume(self):
        if not self.save_full_state:
            return
        self._step_tracker.completed_steps = self.completed_steps
        self.accelerator.register_for_checkpointing(self.lr_scheduler, self._step_tracker)
        if self._full_state_resume_dir is not None:
            self.accelerator.load_state(self._full_state_resume_dir)
            self.completed_steps = self._step_tracker.completed_steps
            logger.info(
                f"[full-resume] restored optimizer+scheduler+RNG+step from "
                f"{self._full_state_resume_dir}, resuming at step {self.completed_steps}"
            )

    def _reseed_dataset_for_resume(self):
        if self.completed_steps <= 0:
            return
        ds = getattr(self.vla_train_dataloader, "dataset", None)
        for _ in range(4):
            if ds is None or hasattr(ds, "reset_seed"):
                break
            ds = getattr(ds, "dataset", None)
        if ds is not None and hasattr(ds, "reset_seed") and hasattr(ds, "seed"):
            new_seed = int(ds.seed) + int(self.completed_steps)
            ds.reset_seed(new_seed)
            logger.info(
                f"[resume-reseed] dataset reseeded to {new_seed} "
                f"(orig_seed + completed_steps={self.completed_steps}) -> fresh data, no repeat"
            )
        else:
            logger.warning("[resume-reseed] dataset has no reset_seed/seed; data may repeat on resume")
    ...
    def _save_checkpoint(self):
        ...
        if self.save_full_state:
            ...
            self.accelerator.print(f"✅ Full training state saved at {state_dir}")
