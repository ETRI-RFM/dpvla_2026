def get_vlm_model(config):
    ...
    interface = str(config.framework.qwenvl.get("interface", "") or "").lower()
    if interface == "qwen3_dpvla":
        from .QWen3_DPVLA import _QWen3_DPVLA_Interface

        return _QWen3_DPVLA_Interface(config)
    elif interface:
        raise NotImplementedError(f"qwenvl.interface '{interface}' not implemented")
